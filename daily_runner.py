...
def _update_open_trades():
    ...
    for _, t in trades[trades.status == "OPEN"].iterrows():
        try:
            ...
            print(f"Trade update error {t.symbol}: {e}")
...
def _open_previous_buy_signals():
    ...
    dates = sorted(
        previous["signal_date"].dropna().unique(),
        reverse=True,
    )
    ...
    for _, r in candidates.head(5).iterrows():
        try:
            ...
            if latest_date > previous_date:
                ...
                # Keep the original BUY signal date
                # so the same signal can never be reused.
                trade["signal_date"] = previous_date

                # Paper entry uses the newer session's open.
